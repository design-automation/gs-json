...
import {parabola_parabola, parabola_width} from "../conic_intersect/parabola";
...
export function genModel_3DConic_Parabola(): gs.IModel {
    ...
    const angle1: number = 90;
    const p: number = 5;
    const parabola: gs.IParabola = m.getGeom().addParabola(center,[p,0,0],[0,1,0],[angle0, angle1]);
    const polyline: gs.IPolyline = parabola_polyline.parabola_polyline(parabola);
    console.log("Width = " + parabola_width(parabola));
...
export function genModel_3DConic_Hyperbola(): gs.IModel {
    ...
    const b: number = t_v2.length();
    const angle_max: number = Math.atan(a/b)*360/(2*Math.PI);
...
export function genModel_ellipse_ellipse(): gs.IModel {
    const m: gs.IModel = new gs.Model();
    const g: gs.IGeom = m.getGeom();
    for(let k: number = 0; k<10; k++) {
    ...
    const V1: three.Vector3 = new three.Vector3(ellipse1.getAxes()[1][0],
                                                ellipse1.getAxes()[1][1],
                                                ellipse1.getAxes()[1][2]).normalize();
    const a: number = ellipse1.getRadii()[0];
    const b: number = ellipse1.getRadii()[1];
    ...
    const ellipse2: gs.IEllipse = m.getGeom().addEllipse(center2,
                                                        [b*U1.x,b*U1.y,b*U1.z],
                                                        [a*V1.x,a*V1.y,a*V1.z],
                                                        ellipse1.getAngles());

    const polyline1: gs.IPolyline = ellipse_polyline.ellipse_polyline_renderXYZ(ellipse1);
    const polyline2: gs.IPolyline = ellipse_polyline.ellipse_polyline_renderXYZ(ellipse2);
    const points: gs.IPoint[] = ellipse_ellipse(ellipse1,ellipse2);
...
export function genModel_plane3D_ellipse2D(): gs.IModel {
    ...
    const V1: three.Vector3 = new three.Vector3(ellipse1.getAxes()[1][0],
                                                ellipse1.getAxes()[1][1],
                                                ellipse1.getAxes()[1][2]).normalize();
    const W1: three.Vector3 = new three.Vector3(ellipse1.getAxes()[2][0],
                                                ellipse1.getAxes()[2][1],
                                                ellipse1.getAxes()[2][2]).normalize();
    const a: number = ellipse1.getRadii()[0];
    const b: number = ellipse1.getRadii()[1];
    const center2: gs.IPoint = g.addPoint([
                        center1.getPosition()[0] + (0.5*a)*V1.x + (0.5*b)*U1.x,
...
export function genModel_plane3D_hyperbola(): gs.IModel {
    const m: gs.IModel = new gs.Model();
    const g: gs.IGeom = m.getGeom();
    for(let k: number = 0; k<0; k++) {
    const center1: gs.IPoint = g.addPoint([40*Math.random(),40*Math.random(),40*Math.random()]);
    const hyperbola1: gs.IHyperbola = m.getGeom().addHyperbola(center1, [5*Math.random(),5*Math.random(),5*Math.random()],
                                                                [15*Math.random(),15*Math.random(),15*Math.random()],
                                                                [0, 360]);
    ...
    const W1: three.Vector3 = new three.Vector3(hyperbola1.getAxes()[2][0],
                                                hyperbola1.getAxes()[2][1],
                                                hyperbola1.getAxes()[2][2]).normalize();
    const a: number = hyperbola1.getRadii()[0];
    const b: number = hyperbola1.getRadii()[1];
    const center2: gs.IPoint = g.addPoint([
                        center1.getPosition()[0] + (0.5*a)*V1.x + (0.5*b)*U1.x,
                        center1.getPosition()[1] + (0.5*a)*V1.y + (0.5*b)*U1.y,
                        center1.getPosition()[2] + (0.5*a)*V1.z + (0.5*b)*U1.z]);
    const plane1: gs.IPlane = g.addPlane(center2,[U1.x,U1.y,U1.z],[W1.x,W1.y,W1.z]);
    const polyline1: gs.IPolyline = hyperbola_polyline.hyperbola_polyline_renderXYZ(hyperbola1);
    const points: gs.IPoint[] = plane3D_hyperbola(hyperbola1,plane1);
    for(const point of points) {
        g.addCircle(point, [0.2*U1.x,0.2*U1.y,0.2*U1.z],[0.2*V1.x,0.2*V1.y,0.2*V1.z]);
    }
...
export function genModel_plane3D_parabola(): gs.IModel {
    const m: gs.IModel = new gs.Model();
    const g: gs.IGeom = m.getGeom();
    for(let k: number = 0; k<1; k++) {
    const center1: gs.IPoint = g.addPoint([40*Math.random(),40*Math.random(),40*Math.random()]);
...
export function genModel_3D_Ray2_circle_2D(): gs.IModel {
    ...
    const points: gs.IPoint[] = rayTwo_circle(ray2, circle);
    for(const point of points) {
        g.addCircle(point, [0.2*U1.x,0.2*U1.y,0.2*U1.z],[0.2*V1.x,0.2*V1.y,0.2*V1.z]);
    }
    }
...
export function genModel_3D_Ray2_hyperbola_2D(): gs.IModel {
    const m: gs.IModel = new gs.Model();
    const g: gs.IGeom = m.getGeom();
    ...
    if (angle0 > angle0_max) {domain_angle1 = 360 - (angle0 - angle1_max);}
    const angle1: number = (angle0 + domain_angle1*Math.random()) %360;
    const hyperbola: gs.IHyperbola = m.getGeom().addHyperbola(
        g.addPoint([40*Math.random(),40*Math.random(),40*Math.random()]),
        vec1,
        vec2,
        [angle0, angle1]);
    ...
    const r1: number = 0.5* (0.5*a + 0.5*b);
    const center_ray2: gs.IPoint = hyperbola.getOrigin();
    const t: number = 1;
    const ray2_direction: gs.XYZ = [ t*U1.normalize().x + (1-t)*V1.normalize().x,
                                     t*U1.normalize().y + (1-t)*V1.normalize().y,
                                     t*U1.normalize().z + (1-t)*V1.normalize().z];
    const ray2: gs.IRayTwo = g.addRayTwo(center_ray2, ray2_direction);
    const polyline1: gs.IPolyline = parabola_polyline.parabola_polyline_renderXYZ(hyperbola);
    const polyline2: gs.IPolyline = rayTwo_polyline(ray2);
    // const points: gs.IPoint[] = rayTwo_parabola(ray2, hyperbola);
    points = rayTwo_parabola(ray2, hyperbola);
    for(const point of points) {
        g.addCircle(point, [0.2*U1.x,0.2*U1.y,0.2*U1.z],[0.2*V1.x,0.2*V1.y,0.2*V1.z]);
    }

import {ICircle, IEllipse, IPolyline, IGeom, IPoint, IModel, XYZ} from "../ifaces_gs";
...
export function circle_polyline(circle: ICircle): IPolyline {
    const points: IPoint[] = [];
    const polyline: IPolyline[] =[];
    const model: IModel = circle.getModel();
    const geometry: IGeom = model.getGeom();
    const a: number = new three.Vector3(circle.getAxes()[0][0],
                                        circle.getAxes()[0][1],
                                        circle.getAxes()[0][2]).length();
    const b: number = new three.Vector3(circle.getAxes()[1][0],
                                        circle.getAxes()[1][1],
                                        circle.getAxes()[1][2]).length();
    const angle0: number = ((circle.getAngles()[0] %360) + 360) %360;
    let angle1: number = (((circle.getAngles()[1] %360) + 360) %360);
    if(angle1<angle0) {angle1 = angle1 + 360;}
    const U1: three.Vector3 = new three.Vector3(circle.getAxes()[0][0],
                                                circle.getAxes()[0][1],
                                                circle.getAxes()[0][2]).normalize();
    const V1: three.Vector3 = new three.Vector3(circle.getAxes()[1][0],
                                                circle.getAxes()[1][1],
                                                circle.getAxes()[1][2]).normalize();
    const center: three.Vector3 = new three.Vector3(circle.getOrigin().getPosition()[0],
                                                   circle.getOrigin().getPosition()[1],
                                                   circle.getOrigin().getPosition()[2]);
    let theta: number = angle0 * (2*Math.PI)/360;
    let r: number;
    const N: number = 50;
    const d_theta: number = (angle1-angle0)/(N)*(2*Math.PI)/360;

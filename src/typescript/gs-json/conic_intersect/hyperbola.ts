import {IHyperbola, IPolyline, IGeom, IPoint, IModel, XYZ, IRayTwo} from "../ifaces_gs";
...
import {rayTwo_hyperbola} from "../conic_intersect/rayTwo";

export function hyperbola_hyperbola(hyperbola1: IHyperbola, hyperbola2: IHyperbola): IPoint[] {
const result: IPoint[] = [];
const xyz_results: XYZ[] = [];
const geom: IGeom =hyperbola1.getGeom();
const eps: number = 1e-300;
const threshold: number = 1e-4;
const angle0_p1: number = ((hyperbola1.getAngles()[0] %360) + 360) %360;
const angle1_p1: number = (((hyperbola1.getAngles()[1] %360) + 360) %360);
const U1: three.Vector3 = new three.Vector3(hyperbola1.getAxes()[0][0],
                                            hyperbola1.getAxes()[0][1],
                                            hyperbola1.getAxes()[0][2]).normalize();
const V1: three.Vector3 = new three.Vector3(hyperbola1.getAxes()[1][0],
                                            hyperbola1.getAxes()[1][1],
                                            hyperbola1.getAxes()[1][2]).normalize();
const p: number = new three.Vector3(hyperbola1.getAxes()[0][0],
                            hyperbola1.getAxes()[0][1],
                            hyperbola1.getAxes()[0][2]).length();
...
const xyz: any[] = Arr.deepCopy(hyperbola1.getOrigin().getPosition());
const center_ray2: IPoint = geom.addPoint([xyz[0],xyz[1],xyz[2]]);
const p2: number = new three.Vector3(hyperbola2.getAxes()[0][0],
                            hyperbola2.getAxes()[0][1],
                            hyperbola2.getAxes()[0][2]).length();
...
for (let k = -2; k<N+2 ; k++) {
    ...
    const ray2: IRayTwo = geom.addRayTwo(center_ray2, hyperbola1.getAxes()[1]);
    const points1: IPoint[] = rayTwo_hyperbola(ray2, hyperbola1);
    const points2: IPoint[] = rayTwo_hyperbola(ray2, hyperbola2);
...
for( const k of list) {
    ...
    let xyz_result: XYZ;
        ...
        count_while_2 = 0;
                ...
                init_k1_y + count_while_2*(width_k1)*U1.y,
                init_k1_z + count_while_2*(width_k1)*U1.z]);
                let ray2: IRayTwo = geom.addRay(center_ray2, hyperbola1.getAxes()[1]);
                if(count_while_1 === 1){
                }
                const points1: IPoint[] = rayTwo_hyperbola(ray2, hyperbola1);
                const points2: IPoint[] = rayTwo_hyperbola(ray2, hyperbola2);
                ...
                if( distance_k1.length >= 2) {
                    ...
                    if(distance_k1[distance_k1.length - 1] >  distance_k1[distance_k1.length - 2]) {
                        ...
                        do {
                            ...
                            ray2 = geom.addRay(center_ray2, hyperbola1.getAxes()[1]);
                            const points10: IPoint[] = rayTwo_hyperbola(ray2, hyperbola1);
                            xyz_result = points10[0].getPosition();
                            const points20: IPoint[] = rayTwo_hyperbola(ray2, hyperbola2);
                            geom.delObj(ray2, false);
                            geom.delPoint(center_ray2);
                            ...
                            ray2 = geom.addRay(center_ray2, hyperbola1.getAxes()[1]);
                            const points11: IPoint[] = rayTwo_hyperbola(ray2, hyperbola1);
                            const points21: IPoint[] = rayTwo_hyperbola(ray2, hyperbola2);
                            geom.delObj(ray2, false);
                            geom.delPoint(center_ray2);

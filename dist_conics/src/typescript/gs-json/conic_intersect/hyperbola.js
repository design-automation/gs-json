...
function hyperbola_hyperbola(hyperbola1, hyperbola2) {
    const result = [];
    const xyz_results = [];
    const geom = hyperbola1.getGeom();
    const eps = 1e-300;
    const threshold = 1e-4;
    const angle0_p1 = ((hyperbola1.getAngles()[0] % 360) + 360) % 360;
    const angle1_p1 = (((hyperbola1.getAngles()[1] % 360) + 360) % 360);
    ...
    const p = new three.Vector3(hyperbola1.getAxes()[0][0], hyperbola1.getAxes()[0][1], hyperbola1.getAxes()[0][2]).length();
    ...
    const xyz = arr_1.Arr.deepCopy(hyperbola1.getOrigin().getPosition());
    const center_ray2 = geom.addPoint([xyz[0], xyz[1], xyz[2]]);
    const p2 = new three.Vector3(hyperbola2.getAxes()[0][0], hyperbola2.getAxes()[0][1], hyperbola2.getAxes()[0][2]).length();
    const N = (p / p2) * 20;
    ...
    for (let k = -2; k < N + 2; k++) {
        ...
        const ray2 = geom.addRayTwo(center_ray2, hyperbola1.getAxes()[1]);
        const points1 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola1);
        const points2 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola2);
            ...
            do {
                ...
                do {
                    center_ray2.setPosition([init_k1_x + count_while_2 * (width_k1) * U1.x,
                        init_k1_y + count_while_2 * (width_k1) * U1.y,
                        init_k1_z + count_while_2 * (width_k1) * U1.z]);
                    let ray2 = geom.addRay(center_ray2, hyperbola1.getAxes()[1]);
                    if (count_while_1 === 1) {
                    }
                    const points1 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola1);
                    const points2 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola2);
                    ...
                    if (distance_k1.length >= 2) {
                        ...
                        if (distance_k1[distance_k1.length - 1] > distance_k1[distance_k1.length - 2]) {
                            ...
                            do {
                                ...
                                const points10 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola1);
                                xyz_result = points10[0].getPosition();
                                const points20 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola2);
                                geom.delObj(ray2, false);
                                geom.delPoint(center_ray2);
                                ...
                                ray2 = geom.addRay(center_ray2, hyperbola1.getAxes()[1]);
                                const points11 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola1);
                                const points21 = rayTwo_1.rayTwo_hyperbola(ray2, hyperbola2);
                                geom.delObj(ray2, false);
                                geom.delPoint(center_ray2);
